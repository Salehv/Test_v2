...
public class GameManager : DynamicsHandler
{
    ...
    internal DynamicsFlag LevelDynamicsFlag
    {
        get { return currentLevel.flags; }
    ...
    private static void InitDictionaries()
    {
        ...
        dic_charToLetter.Add(' ', -1);
        dic_charToLetter.Add('ا', 0);
        dic_charToLetter.Add('آ', 0);
        dic_charToLetter.Add('ب', 1);
        dic_charToLetter.Add('پ', 2);
        dic_charToLetter.Add('ت', 3);
        dic_charToLetter.Add('ث', 4);
        dic_charToLetter.Add('ج', 5);
        dic_charToLetter.Add('چ', 6);
        dic_charToLetter.Add('ح', 7);
        dic_charToLetter.Add('خ', 8);
        dic_charToLetter.Add('د', 9);
        dic_charToLetter.Add('ذ', 10);
        dic_charToLetter.Add('ر', 11);
        dic_charToLetter.Add('ز', 12);
        dic_charToLetter.Add('ژ', 13);
        dic_charToLetter.Add('س', 14);
        dic_charToLetter.Add('ش', 15);
        dic_charToLetter.Add('ص', 16);
        dic_charToLetter.Add('ض', 17);
        dic_charToLetter.Add('ط', 18);
        dic_charToLetter.Add('ظ', 19);
        dic_charToLetter.Add('ع', 20);
        dic_charToLetter.Add('غ', 21);
        dic_charToLetter.Add('ف', 22);
        dic_charToLetter.Add('ق', 23);


        dic_charToLetter.Add('\u06A9', 24);
        dic_charToLetter.Add('\uFB8E', 24);
        dic_charToLetter.Add('\uFB8F', 24);
        dic_charToLetter.Add('\uFB90', 24);
        dic_charToLetter.Add('\uFB91', 24);


        dic_charToLetter.Add('گ', 25);
        dic_charToLetter.Add('ل', 26);
        dic_charToLetter.Add('م', 27);
        dic_charToLetter.Add('ن', 28);
        dic_charToLetter.Add('و', 29);
        dic_charToLetter.Add('ه', 30);

        dic_charToLetter.Add('\u064A', 31); // ی
        dic_charToLetter.Add('\u0649', 31);
        dic_charToLetter.Add('\uFEF1', 31);
        dic_charToLetter.Add('\uFEF2', 31);
        dic_charToLetter.Add('\uFEF3', 31);
        dic_charToLetter.Add('\uFEF4', 31);
        dic_charToLetter.Add('\uFEEF', 31);
        dic_charToLetter.Add('\uFEF0', 31);
        dic_charToLetter.Add('\u06CC', 31);

        dic_letterToChar = new Dictionary<int, char>();
        dic_letterToChar.Add(-1, ' ');
        dic_letterToChar.Add(0, 'ا');
        dic_letterToChar.Add(1, 'ب');
        dic_letterToChar.Add(2, 'پ');
        dic_letterToChar.Add(3, 'ت');
        dic_letterToChar.Add(4, 'ث');
        dic_letterToChar.Add(5, 'ج');
        dic_letterToChar.Add(6, 'چ');
        dic_letterToChar.Add(7, 'ح');
        dic_letterToChar.Add(8, 'خ');
        dic_letterToChar.Add(9, 'د');
        dic_letterToChar.Add(10, 'ذ');
        dic_letterToChar.Add(11, 'ر');
        dic_letterToChar.Add(12, 'ز');
        dic_letterToChar.Add(13, 'ژ');
        dic_letterToChar.Add(14, 'س');
        dic_letterToChar.Add(15, 'ش');
        dic_letterToChar.Add(16, 'ص');
        dic_letterToChar.Add(17, 'ض');
        dic_letterToChar.Add(18, 'ط');
        dic_letterToChar.Add(19, 'ظ');
        dic_letterToChar.Add(20, 'ع');
        dic_letterToChar.Add(21, 'غ');
        dic_letterToChar.Add(22, 'ف');
        dic_letterToChar.Add(23, 'ق');
        dic_letterToChar.Add(24, 'ک');
        dic_letterToChar.Add(25, 'گ');
        dic_letterToChar.Add(26, 'ل');
        dic_letterToChar.Add(27, 'م');
        dic_letterToChar.Add(28, 'ن');
        dic_letterToChar.Add(29, 'و');
        dic_letterToChar.Add(30, 'ه');
        dic_letterToChar.Add(31, '\u06CC');
    }
    ...
    internal void PlayLevel(Level lvl)
    {
        ...
        TutorialHandler.instance.TUT01.SetActive(false);
        ...
        words.AddFirst(lvl.begin);
        currentEndWord = NormalizeText(lvl.end);

        ResetDynamics();
        int[] related = GetRelatedChars(NormalizeText(lvl.begin), currentShufflePage);
        letterPool.Init(textEditor, related);


        for (int i = 0; i < endWord.childCount; i++)
        {
            Destroy(endWord.GetChild(i).gameObject);
        }

        for (int i = lvl.end.Length - 1; i >= 0; i--)
        {
            EndWordHandler.instance.CreateLetter(NormalizeText(lvl.end)[i]);
        }
        ...
        // UI
        inGameBack.sprite = ResourceManager.instance.GetChapterInGameBackground(lvl.chapterId);
        inGameFeatures.sprite = ResourceManager.instance.GetChapterInGameFeaturesSprite(lvl.chapterId);
        ...
        {
            AddCorrectWord(NormalizeText("کن"));
    ...
    {
        if (words.Last.Value != NormalizeText(currentLevel.end))
            return;
        Win();
    }
    ...
    {
        return NormalizeText(words.Last.Value);
    }
    ...
    {
        if (debugMode)
            Debug.Log("Change Letter at [" + inTextPosition + "] to " + dic_letterToChar[code]);

        string s = GetLastWord();

        if (inTextPosition == 0)
            s = dic_letterToChar[code] + s.Substring(inTextPosition + 1);
        else if (inTextPosition == s.Length - 1)
            s = s.Substring(0, inTextPosition) + dic_letterToChar[code];
        else
            s = s.Substring(0, inTextPosition) + dic_letterToChar[code] + s.Substring(inTextPosition + 1);
        ...
        {
            if (s == NormalizeText(words.Last.Value))
                return false;
    ...
    {
        if (debugMode)
            Debug.Log("Add Letter [" + dic_letterToChar[code] + "] between [" + inTextPosition + ", " +
                      (inTextPosition + 1) + "]");

        string s = GetLastWord();
        s = s.Substring(0, inTextPosition) + dic_letterToChar[code] + s.Substring(inTextPosition);
        s = NormalizeText(s);
    ...
    public void Undo()
    {
        ...
        words.RemoveLast();
        Destroy(wordsView.transform.GetChild(wordsView.transform.childCount - 1).gameObject);
    ...
    private void AddCorrectWord(string s)
    {
        ...
        words.AddLast(s);
        AddWordToWordsView(words.Last.Previous.Value);
        letterPool.Init(textEditor, GetRelatedChars(s, currentShufflePage));
        textEditor.ResetPluses();
        CheckWin();
    }

    private void AddWordToWordsView(string s)
    {
    ...
    public void ShuffleLetterSelected(int letter)
    {
        ...
        middle.GetComponent<AvailableLetter>().Init(textEditor, dic_letterToChar[letter]);
    ...
    public void Hint_ShowSimilarWords()
    {
        ...
        var sWords = DatabaseManager.instance.GetAllSimilarWords(NormalizeText(words.Last.Value));
        ...
        foreach (var word in sWords)
        {
            ...
            shownSimilars[c] = NormalizeText(word);
            c++;
        }
    }
    ...
    internal static string NormalizeText(string s)
    {
        ...
        for (int i = 0; i < s.Length; i++)
        {
            try
            {
                ...
                t = t + dic_letterToChar[dic_charToLetter[s[i]]];
            }
            catch (KeyNotFoundException e)
            {
                Debug.LogError("Letter is not recognized [GameManager-NormalizeText] " + s + ":" + t);
                t += s[i];
            }
        }
    ...
    private int[] GetRelatedChars(string word, int page)
    {
        ...
        for (int i = 0; i < prior.Length; i++)
        {
            availableChars.Add(dic_charToLetter[prior[i]]);
        }
        ...
        {
            if (!availableChars.Contains(dic_charToLetter[possibleChars[i]]))
            {
                availableChars.Add(dic_charToLetter[possibleChars[i]]);
            }
        }
